 ...
 */
public class GAStrategy extends AStrategy {

	private double m_aI8_e;//一个很小的数
	private int m_aI4_sf;
	private double m_aI8_b;
	private double m_aI8_pc;// 交叉概率
	private double m_aI8_pm;// 变异概率
	public GASolution m_aTC_best;// 全局最优解
	private GASolution[] m_aTC_newPopulation;// 子种群，从父种群中选取，进行交叉和变异

	public GAStrategy() {
	}

	public GAStrategy(int f_aI4_size, int f_aI4_max_nfe, int f_aI4_max_iter, SSModel f_aTC_ssm, String f_str_data_path, long f_aI8_max_time) {
		super(f_aI4_size, f_aI4_max_nfe, f_aI4_max_iter, f_aTC_ssm, f_str_data_path, f_aI8_max_time);
	}

	@Override
	protected void init() {
		this.m_aTC_population = new GASolution[m_aI4_size];// 父代
		this.m_aTC_newPopulation = new GASolution[m_aI4_size];// 子代
	...
	private void initBest() {
		// 全局最优解的初始化设置
		m_aTC_best = new GASolution(m_aI4_d);
		...
		for(int t_aI4_i=0;t_aI4_i<m_aI4_size;t_aI4_i++){
			if(Double.compare(m_aTC_best.getM_aI8_fitness(), m_aTC_population[t_aI4_i].getM_aI8_fitness())<0){
				m_aTC_best = ((GASolution) m_aTC_population[t_aI4_i]).clone();
			}
		}
	}
	...
	private void selectBest(){
		int t_aI4_num = (int) (m_aI8_b * m_aI4_size);
		boolean t_rI1_visit[] = new boolean[m_aI4_size];
		for(int t_aI4_i=0;t_aI4_i<t_aI4_num;t_aI4_i++){
			int t_aI4_maxid = 0;
			double t_aI8_maxFitness = m_aTC_population[0].getM_aI8_fitness();
			
			for(int t_aI4_j=1;t_aI4_j<m_aI4_size;t_aI4_j++){
				if(!t_rI1_visit[t_aI4_j] && Double.compare(m_aTC_population[t_aI4_j].getM_aI8_fitness(), t_aI8_maxFitness) > 0){
					t_aI8_maxFitness = m_aTC_population[t_aI4_j].getM_aI8_fitness();
					t_aI4_maxid = t_aI4_j;
				}
			}
			
			if(Double.compare(m_aTC_best.getM_aI8_fitness(), t_aI8_maxFitness) < 0){
				m_aTC_best = ((GASolution) m_aTC_population[t_aI4_maxid]).clone();
			}
			t_rI1_visit[t_aI4_maxid] = true;
			//将当代种群中适应度最高的染色体maxid复制到新种群中，排在第一位
			copySolution(t_aI4_i, t_aI4_maxid);
		}
	...
	private void copySolution(int f_aI4_k1,int f_aI4_k2){
		m_aTC_newPopulation[f_aI4_k1] = ((GASolution) m_aTC_population[f_aI4_k2]).clone();
	}
	...
	private void select(){
		...
		int t_aI4_num = (int) (m_aI8_b * m_aI4_size);
		for(int t_aI4_i=t_aI4_num;t_aI4_i<m_aI4_size;t_aI4_i++){//最优的已经直接复制到子代，所以此处再选择n-1个就行
	...
	private void variation(int f_aI4_k){
		...
		int count = m_aTC_random.nextInt(m_aI4_k);//对换次数
		GASolution solution = m_aTC_newPopulation[f_aI4_k];
		int[] t_rI4_x = solution.getM_rI4_x();

		for(int i=0;i<count;i++){
			t_aI4_x = m_aTC_random.nextInt(65535) % m_aI4_d;
			t_aI4_y = m_aTC_random.nextInt(65535) % m_aI4_d;
			while(t_aI4_x==t_aI4_y || t_aI4_x%2!=t_aI4_y%2){
				t_aI4_y = m_aTC_random.nextInt(65535) % m_aI4_d;
			}
	...
	public static void main(String[] args) {
		...
		AStrategy t_aTC_strategy = new GAStrategy(10, 1000, 500, t_aTC_ssm, NameSpace.s_str_data_01,1000);
