...
public abstract class AStrategy {
	...
	protected void modify(ASolution f_aTC_s) {
		...
		//System.out.println("t_aI4_count = "+t_aI4_count);
		//System.out.println("修正完成");
	}
	...
	private List<Integer> getPre(int f_aI4_j,int[] f_rI4_x){
		...
		for(int t_aI4_i=0;t_aI4_i<m_aI4_d;t_aI4_i++){
			if(t_aI4_i%2==0){
				if(f_rI4_x[f_aI4_j*2]==f_rI4_x[t_aI4_i] && Double.compare(m_rI8_t0j[t_aI4_i/2], m_rI8_t0j[f_aI4_j])<0)
	...
	private List<Integer> getPre2(int f_aI4_j,int[] f_rI4_x,double[] f_rI8_t1){
		...
		for(int t_aI4_i=0;t_aI4_i<m_aI4_d;t_aI4_i++){
			if(t_aI4_i%2!=0){
				...
				if(t_aI4_yj !=0 && f_rI4_x[f_aI4_j*2+1]==f_rI4_x[t_aI4_i] && Double.compare(f_rI8_t1[(t_aI4_i-1)/2], f_rI8_t1[f_aI4_j])<0)
	...
	private double calT2(ASolution f_aTC_s){
		...
		double t_aI8_t = 0;
		for(int t_aI4_j=0;t_aI4_j<m_aI4_n;t_aI4_j++){//对每个行李
			int t_aI4_yj = t_rI4_x[t_aI4_j * 2+1];// 决策变量
			t_rI8_t2[t_aI4_j] += t_rI8_t1j[t_aI4_j];
			//构造Pre(j)集合
			List<Integer> t_aTC_pre = getPre2(t_aI4_j, t_rI4_x,t_rI8_t1j);
			for(int t_aI4_i=0;t_aI4_i<t_aTC_pre.size();t_aI4_i++){
				t_rI8_t2[t_aI4_j] += m_rI8_tj[t_aTC_pre.get(t_aI4_i)];
			}
	...
	protected String getResultContent(int f_aI4_p, ASolution f_aTC_solution) {
		...
		t_aTC_sb.append("fitness = " + f_aTC_solution.getM_aI8_fitness()+"|time = "+f_aTC_solution.getM_aI8_maxtime()+"|"+isValidSolution(f_aTC_solution)+"\n");
	...
	protected void printSolution(ASolution f_aTC_solution) {
		...
		System.out.println("fitness = " + f_aTC_solution.getM_aI8_fitness()+"|time = "+f_aTC_solution.getM_aI8_maxtime()+"|"+isValidSolution(f_aTC_solution));
	}
