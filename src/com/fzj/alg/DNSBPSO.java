 ...
 */
public class DNSBPSO extends AStrategy {
	...
	public DNSBPSO(int f_aI4_size, int f_aI4_max_nfe, int f_aI4_max_iter, SSModel f_aTC_ssm, String f_str_data_path) {
		...
		while (m_aI4_cur_nfe < m_aI4_max_nfe) {
			// 第一阶段
			{
				...
				for (int t_aI4_i = 0; t_aI4_i < m_aI4_size; t_aI4_i++) {
					ASolution t_aTC_pit = m_aTC_population[t_aI4_i];
					// 生成粒子Pi(t)
					updateXV(t_aTC_pit, t_aI4_i);// 计算粒子Pi的速度和位移
					modify(t_aTC_pit);
					// 生成粒子Pi(t+1)
					ASolution t_aTC_pit2 = t_aTC_pit.clone();
					...
					m_aI4_cur_nfe++;
					// 开始多样性提高机制
					ASolution t_aTC_tpi2 = dem(t_aTC_pit, t_aTC_pit2);// 评价次数增加
	 ...
	 * @return
	 */
	private ASolution dem(ASolution f_aTC_pit, ASolution f_aTC_pit2) {
		// 生成新的粒子TPi2
		ASolution t_aTC_TPi2 = f_aTC_pit2.clone();
		int[] t_rI4_txi2 = new int[m_aI4_d];
		double[] t_rI4_tvi2 = new double[m_aI4_d];

		int[] t_rI4_xit = f_aTC_pit.getM_rI4_x();
		int[] t_rI4_xit2 = f_aTC_pit2.getM_rI4_x();
		double[] t_rI4_vit2 = f_aTC_pit2.getM_rI8_v();
		...
		t_aTC_TPi2.setM_rI8_v(t_rI4_tvi2);
		modify(t_aTC_TPi2);
		evaluate(t_aTC_TPi2);
		saveFitness(m_aTC_gBest);
		m_aI4_cur_nfe++;
